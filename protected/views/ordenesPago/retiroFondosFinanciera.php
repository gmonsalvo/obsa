...
<script>
    ...
    function MostrarInformacion(){
        if($("#OrdenesPago_financieraId").val()==""){
            return false;
        } else {
            $.post("/obsa/index.php/financieras/getSaldos?render=false",
                {"id":$("#OrdenesPago_financieraId").val(), "productoId":$("#OrdenesPago_productoId").val()},function(data){
            if(data==""){
...
<div class="form">
    ...
    <div class="row">
        <?php
        echo $form->labelEx($model, 'financieraId');
        $this->widget('CustomEJuiAutoCompleteFkField', array(
            'model' => $model,
            'attribute' => 'financieraId', //the FK field (from CJuiInputWidget)
...
//            'javascript' => "alert('holaaa');",
        ...
        echo CHtml::ajaxButton('Mostrar informacion', CHtml::normalizeUrl(array('financieras/getSaldos', 'render' => false)), array(
            ...
            'beforeSend' => 'js:function(){
                    if($("#OrdenesPago_financieraId").val()==""){
                        alert("Debe seleccionar una financiera");
                        return false;
                    }
                    return true;
                }',
